...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Plantilla de descripción (para no repetir el mismo texto)
    const descriptionTemplate = `PIJAMA SHORT TIRAS
...
-Estamos en Medellín centro.`;
    ...
    const pageFlip = new St.PageFlip(bookElement, {
        width: bookContainer.clientWidth / 2, // la mitad del ancho del libro en CSS
        height: bookContainer.clientHeight, // la altura completa del libro en CSS
        showCover: true, // Muestra la primera página como una portada
        usePortrait: false, // Asegura que el modo sea siempre apaisado
    });

    // --- CONSTRUCCIÓN DINÁMICA DEL LIBRO ---

    // 1. Añadir una portada
    ...
    // 2. Añadir las páginas de los productos
    products.forEach(product => {
        // Página de la imagen (izquierda)
        const imgPage = document.createElement('div');
        imgPage.classList.add('page', 'image-page');
        imgPage.innerHTML = `<img src="${product.image}" alt="Pijama Ref ${product.ref}">`;
        bookElement.appendChild(imgPage);

        // Página de los detalles (derecha)
        const detailsPage = document.createElement('div');
        detailsPage.classList.add('page', 'details-page');
        detailsPage.innerHTML = `
            <div class="product-details">
                <h2 class="product-title">Pijama Short Ref: ${product.ref}</h2>
                <p class="product-price">Desde COP 9,000.00</p>
                <div class="product-description">${descriptionTemplate}</div>
        ...
        `;
        bookElement.appendChild(detailsPage);
    });
    
    // 3. Cargar las páginas en la librería
    // Usamos el método loadFromHTML porque nuestras páginas son divs complejos
